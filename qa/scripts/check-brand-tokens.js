...
async function checkBrandCompliance(cssFiles) {
  ...
  for (const cssFile of cssFiles) {
    ...
    matches.forEach((match) => {
      ...
      const isPvaColor = Object.values(PVA_COLORS).some(
        (color) => color.toLowerCase() === lowerMatch,
      );
...
async function checkFontCompliance(cssFiles) {
  ...
  const forbiddenFonts = ['Comic Sans MS', 'Papyrus', 'Impact'];
...
async function checkBrandTokens() {
  ...
  const missingColors = colorResults.requiredColors.filter(
    (color) => !colorResults.foundColors.includes(color),
  );
