...
class RoyaltyTracker {
  ...
  recordRoyaltyEvent({
    ...
    const royaltyAmount = normalizedSaleType === 'SECONDARY'
      ? (numericSalePrice * effectiveRate) / 100
      : 0;

    const creatorEarningAmount = normalizedSaleType === 'PRIMARY'
      ? numericSalePrice
      : royaltyAmount;
  ...
  getRoyaltyHistory(creatorAddress, { limit = 100, offset = 0 } = {}) {
    ...
    const total = db.prepare('SELECT COUNT(*) AS count FROM royalty_events WHERE creator_address = ?').get(normalizedCreator);
  ...
  getAllEvents({ limit = 200, offset = 0, platform = '' } = {}) {
    ...
    const total = normalizedPlatform
      ? db.prepare('SELECT COUNT(*) AS count FROM royalty_events WHERE platform = ?').get(normalizedPlatform)
