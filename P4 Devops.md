...
jobs:  
 ...
 run: npm test  
 env:  
 DATABASE_URL: ${{ secrets.TEST\_DATABASE\_URL }}  
 NEXTAUTH_SECRET: ${{ secrets.TEST\_NEXTAUTH\_SECRET }}  
...
jobs:  
 ...
 env:  
 DATABASE_URL: ${{ github.ref\_name \== 'main' && secrets.DATABASE\_URL\_PROD || secrets.DATABASE\_URL\_STAGING }}  
...
resource "aws_lambda_function" "api" {  
 function_name \= "pvabazaar-api-${var.environment}"  
 ...
 PVA_PROJECT \= var.environment \== "production" ? "com" : "org"  
 }  
 }  
}
...
resource "null_resource" "supabase_setup" {  
 ...
 command \= \<\<EOT  
 supabase init  
 supabase db push \--db-url ${var.database_url}  
 supabase functions deploy sync-provenance \--project-ref ${var.supabase_project_ref}  
 EOT  
 environment \= {  
...
terraform  
•\`\`\`hcl  
resource "aws_secretsmanager_secret" "pvabazaar_secrets" {  
 name \= "pvabazaar-secrets-${var.environment}"  
}

resource "aws_secretsmanager_secret_version" "pvabazaar_secrets_version" {  
 secret_id \= aws_secretsmanager_secret.pvabazaar_secrets.id  
...
{  
 ...
 "requests": \[  
 {  
 "q": "avg:supabase.db.response_time{pvabazaar}.rollup(avg, 60)",  
 ...
 \],  
 "layout_type": "ordered",  
 "notify_list": \["team@pvabazaar.com"\],  
...
vercel alias set pvabazaar-$VERCEL_GIT_COMMIT_REF.vercel.app pvabazaar.com \--token $VERCEL\_TOKEN  
curl \-f https://pvabazaar.com/api/health || vercel alias set pvabazaar-$PREV_COMMIT.vercel.app pvabazaar.com \--token $VERCEL_TOKEN  
\`\`\`  
\`\`\`json  
{  
 "detectors": \["all"\],  
...
export function trackArchetypeConversion(userId: string, archetype: string, purchaseId?: string) {  
 Analytics.track('archetype_conversion', {  
 userId,  
 archetype,  
 purchaseId,  
