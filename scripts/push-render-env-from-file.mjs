...
function parseArgs(argv) {
  const out = {
    dryRun: false,
    allowLocalhostMongo: false,
    envFile: path.join(REPO_ROOT, 'render-dashboard.env'),
  };
  for (const a of argv) {
...
if (!args.allowLocalhostMongo && /localhost|127\.0\.0\.1/.test(mongo)) {
  throw new Error(
    'MONGODB_URI points at localhost; fix render-dashboard.env or pass --allow-localhost-mongo.',
  );
}
