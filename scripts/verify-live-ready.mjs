...
if (ADMIN_USERNAME && ADMIN_PASSWORD) {
  ...
  if (!adminLogin.res.ok || adminLogin.json?.ok !== true || !adminLogin.json?.token) {
    ...
    const queueStats = await requestJson(`${BACKEND}/api/openclaw/queue-stats`, { headers: authHeaders });
    if (!queueStats.res.ok || queueStats.json?.ok !== true) fail(`/api/openclaw/queue-stats failed (${queueStats.res.status})`);
    else console.log(`✅ openclaw queue-stats ok (pending=${queueStats.json.pendingOutbound ?? 0}, stale=${queueStats.json.staleOutbound ?? 0})`);
    ...
    if (!bountyStats.res.ok || bountyStats.json?.ok !== true) fail(`/api/bounties/stats failed (${bountyStats.res.status})`);
...
const assetPaths = [...home.text.matchAll(/\/assets\/[^"']+\.(?:js|css)/g)].map(match => match[0]);
...
if (!runtimeCfg || !runtimeCfg.res.ok || typeof runtimeCfg.json?.apiUrl !== "string") {
  ...
  if (apiRoot !== API_BASE) fail(`runtime api-base.json drift detected (${apiRoot} != ${API_BASE})`);

  const pingRuntime = await requestJson(`${apiRoot}/ping`);
  if (!pingRuntime.res.ok || pingRuntime.json?.ok !== true) {
    fail(`runtime api-base.json points to a dead API (${apiRoot}/ping -> ${pingRuntime.res.status})`);
  } else {
...
if (!localShortSha) {
  ...
  if (PARITY_REQUIRED) softWarn("live /api/version has no usable sha/shortSha; cannot prove parity.");
