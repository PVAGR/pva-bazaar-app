    ...
    <section className="w-full space-y-6">
      <header className="space-y-3 rounded-xl border border-zinc-800/80 bg-zinc-950/60 p-6">
        ...
        <p className="max-w-3xl text-sm text-zinc-400">
          Public live testing surface for Meow API integration on PVA Bazaar. Use sandbox keys
          first. This page calls the backend routes under <code>/api/meow</code>.
        </p>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <button
          ...
          onClick={() =>
            call(
              `/api/meow/balances${accountId ? `?accountId=${encodeURIComponent(accountId)}` : ''}`,
            )
          }
          disabled={loading}
        >
      ...
      <div className="rounded-xl border border-zinc-800/80 bg-zinc-950/60 p-5">
        ...
        <div className="mt-3">
          <button
            ...
            onClick={() =>
              call(
                `/api/meow/transactions${accountId ? `?accountId=${encodeURIComponent(accountId)}` : ''}`,
              )
            }
            disabled={loading}
          >
      ...
      {error ? (
        <div className="rounded border border-red-700/70 bg-red-950/30 px-4 py-3 text-sm text-red-300">
          {error}
        </div>
      ) : null}
