  ...
  async function handleLookup(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(
        `${base}/api/verification/artifact/${encodeURIComponent(idOrSlug.trim())}`,
      );
  ...
  return (
    <article className="rounded-xl border border-zinc-800/80 bg-zinc-950/60 p-5">
      ...
      <p className="mb-4 text-xs text-zinc-400">
        Enter an artifact ID or slug to see the latest verification record (hash, status,
        certificate).
      </p>
          ...
          {result.verification ? (
            <>
              <p>
                ...
                {result.verification.certificateId}
              </p>
              <p>
                <span className="text-zinc-500">Status:</span> {result.verification.status}
              </p>
              <p>
                <span className="text-zinc-500">Authentic:</span>{' '}
                {result.verification.is_authentic ? 'Yes' : 'No'}
              </p>
              <p>
              ...
              {result.verification.computed_hash && (
                <p>
                  <span className="text-zinc-500">Hash:</span> {result.verification.computed_hash}
                </p>
              )}
            ...
            <p className="text-zinc-500">
              {result.message ?? 'No verification record for this artifact.'}
            </p>
          )}
        </div>
      )}
