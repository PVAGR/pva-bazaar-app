...
- The book publishing workspace now accepts DOCX and PDF manuscripts through a visible mobile-friendly upload button plus visible file control, with immediate DOCX extraction in the browser, server-side extraction, binary uploads skipped on the client text editor until save, and a free file-backed book store fallback when Mongo is unavailable so publishing does not stall.
