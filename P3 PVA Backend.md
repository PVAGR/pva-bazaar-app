...
This backend is production-ready, TypeScript-based, using Next.js 14 App Router for API routes. It supports both sites via shared codebase with project-specific configs (e.g., env vars for PVA_PROJECT=com or org). Automation focuses on webhooks, cron jobs (via Vercel Cron), and Supabase Edge Functions for blockchain events. All code is GitHub Copilot-friendly with comments.
...
model Transaction { // Blockchain tx for .org  
 ...
 type String // mint, transfer, buy_fraction  
...
// Insert provenance event  
 const { error } \= await supabase.from('provenance_events').insert({  
