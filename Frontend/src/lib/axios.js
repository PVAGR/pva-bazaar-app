  ...
  async (error) => {
    ...
    if (status === 401) {
      ...
      const adminShellSession =
        typeof window !== 'undefined' && sessionStorage.getItem('admin-auth') === 'authenticated';
    ...
    if (status >= 500) {
      const msg = error?.response?.data?.message || error?.message || `Server error (${status})`;
...
api.post = (url, data, config = {}) =>
  requestWithFailover({ ...config, method: 'post', url, data });
api.put = (url, data, config = {}) => requestWithFailover({ ...config, method: 'put', url, data });
api.patch = (url, data, config = {}) =>
  requestWithFailover({ ...config, method: 'patch', url, data });
