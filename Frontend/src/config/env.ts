...
const rawApiUrl = (import.meta.env.VITE_API_URL || "https://pva-backend-api.vercel.app/api").replace(
  /\/$/, ""
);
...
if (isProdBuild && isLocalApi) {
  throw new Error(
    "VITE_API_URL points to localhost in production build. Set it to the deployed API domain (e.g., https://pva-backend-api.vercel.app/api)."
  );
}
