  ...
  const onboardingDraft = onboardingEntry
    ? {
      ...
      solution: [
        ...
        tagList.length ? `Align first actions with selected path tags: ${tagList.join(', ')}.` : '',
        'Assign clear first-week actions and publish completion metrics.',
      ].filter(Boolean).join(' '),
      outcome: 'Higher onboarding-to-contribution conversion and faster first civic participation.',
    }
    : null
    ...
    <>
      {onboardingEntry ? (
        <section className="forum-onboarding-banner section-card" aria-label="Onboarding context">
          ...
          <p>
            We prefilled forum search with your path context so you can find relevant proposals faster.
            {focus ? ` Focus: ${focus}.` : ''}
          </p>
          <div className="row rowWrap">
            <Link className="btn ghost" to="/proposals/submit">Submit your first proposal</Link>
            <Link className="btn ghost" to="/conference">Open conference</Link>
          </div>
        </section>
      ) : null}
      <GovernanceInterface
