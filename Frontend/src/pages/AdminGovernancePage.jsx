...
function toLifecycleStatus(decision) {
  ...
  if (
    ['accepted', 'rejected', 'needs_revision', 'in_execution', 'completed'].includes(normalized)
  ) {
...
export default function AdminGovernancePage() {
  ...
  useEffect(() => {
    ...
    const loadAdminResponses = async () => {
      try {
        const [data] = await Promise.all([fetchGovernanceAdminResponses(), loadSyncHealth()]);
        if (cancelled) return;
  ...
  const getExecutionForm = (proposal) => {
    return (
      executionForms[proposal.id] || {
        owner: proposal.executionProject?.owner || '',
        milestonesText: (proposal.executionProject?.milestones || [])
  ...
  const applyDecision = async (proposal) => {
    ...
    try {
      ...
      setSyncError(
        'Backend persistence is unavailable right now. Decision was applied locally for continuity but is not yet authoritative across devices.',
      );
  ...
  const applyExecution = async (proposal) => {
    ...
    try {
      ...
      setSyncError(
        'Backend persistence is unavailable right now. Execution updates were saved locally for continuity but are not yet authoritative across devices.',
      );
    }
    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '24px 16px' }}>
      ...
      >
        ...
        <p style={{ margin: '8px 0 0', color: 'var(--site-text-muted)', fontSize: '13px' }}>
          Total: {syncHealth.summary.total} · Synced: {syncHealth.summary.synced} · Mismatch:{' '}
          {syncHealth.summary.mismatch} · Missing: {syncHealth.summary.missing} · Local-only:{' '}
          {syncHealth.summary.localOnly}
        </p>
        <div style={{ marginTop: '10px', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
          {[
            ...
            <button
              ...
              style={{
                ...
                border: '1px solid var(--site-border)',
                background:
                  syncHealthFilter === chip.value ? 'var(--site-accent)' : 'var(--site-panel-soft)',
          ...
          <button
            ...
            style={{
              ...
              color: '#fff',
              cursor:
                repairAllLoading || syncHealth.summary.mismatch < 1 ? 'not-allowed' : 'pointer',
        ...
        {visibleSyncItems.length ? (
          <div style={{ marginTop: '10px', display: 'grid', gap: '8px' }}>
            {visibleSyncItems.slice(0, 6).map((item) => (
              <div
              ...
              >
                <span style={{ color: 'var(--site-text-muted)', fontSize: '12px' }}>
                  {item.proposalId}: {item.actualLifecycleStatus || 'n/a'} {'->'}{' '}
                  {item.expectedLifecycleStatus || 'n/a'}
                </span>
                <button
                  type="button"
                  onClick={() => repairLifecycle(item.proposalId)}
                  disabled={
                    Boolean(repairingByProposalId[item.proposalId]) ||
                ...
                >
                  {item.syncState === 'mismatch'
                    ? repairingByProposalId[item.proposalId]
                      ? 'Repairing…'
                      : 'Repair'
                    : 'No Action'}
                </button>
      ...
      <div style={{ display: 'grid', gap: '14px' }}>
        {filteredProposals.map((proposal) => {
          ...
          const healthItem = syncHealth.items.find(
            (item) => item.proposalId === String(proposal._id || proposal.id),
          );
          ...
          return (
            ...
            >
              ...
              {healthBadge ? (
                <p
                  style={{ margin: '0 0 12px', color: 'var(--site-text-muted)', fontSize: '12px' }}
                >
                  Sync: {healthBadge}
                </p>
              ) : null}
              ...
              <div style={{ display: 'grid', gap: '10px', marginBottom: '12px' }}>
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateDecisionField(proposal.id, 'decision', event.target.value)
                    }
                    style={{
                      width: '100%',
                      padding: '8px',
                ...
                <label>
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      updateDecisionField(proposal.id, 'reason', event.target.value)
                    }
                    style={{
                      width: '100%',
                      minHeight: '78px',
                ...
                >
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateDecisionField(proposal.id, 'nextStep', event.target.value)
                      }
                      style={{
                        width: '100%',
                        padding: '8px',
                  ...
                  <label>
                    ...
                    <input
                      ...
                      onChange={(event) =>
                        updateDecisionField(proposal.id, 'targetTimeline', event.target.value)
                      }
                      style={{
                        width: '100%',
                        padding: '8px',
              ...
              {['accepted', 'in_execution', 'completed'].includes(proposal.status) ||
                ...
                <div style={{ borderTop: '1px dashed var(--site-border)', paddingTop: '12px' }}>
                  ...
                  <div style={{ display: 'grid', gap: '10px' }}>
                    <label>
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          updateExecutionField(proposal.id, 'owner', event.target.value)
                        }
                        style={{
                          width: '100%',
                          padding: '8px',
                    ...
                    <label>
                      ...
                      <textarea
                        ...
                        onChange={(event) =>
                          updateExecutionField(proposal.id, 'milestonesText', event.target.value)
                        }
                        style={{
                          width: '100%',
                          minHeight: '76px',
                    ...
                    >
                      <label>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            updateExecutionField(proposal.id, 'progressPercent', event.target.value)
                          }
                          style={{
                            width: '100%',
                            padding: '8px',
                      ...
                      >
                        <input
                          ...
                          onChange={(event) =>
                            updateExecutionField(proposal.id, 'completed', event.target.checked)
                          }
                        />
                        Completed
                      </label>
                    ...
                    <label>
                      ...
                      <textarea
                        ...
                        onChange={(event) =>
                          updateExecutionField(proposal.id, 'latestUpdate', event.target.value)
                        }
                        style={{
                          width: '100%',
                          minHeight: '76px',
                  ...
                  {(timelines[proposal.id]?.updates || []).length ? (
                    <div
                      style={{
                        marginTop: '12px',
                        borderTop: '1px dashed var(--site-border)',
                        paddingTop: '10px',
                      }}
                    >
                      <h4 style={{ margin: '0 0 8px', fontSize: '14px' }}>Execution History</h4>
                      <div style={{ display: 'grid', gap: '8px' }}>
                        {timelines[proposal.id].updates
                          .slice(-5)
                          .reverse()
                          .map((entry, idx) => (
                            <div
                              key={`${proposal.id}-update-${idx}`}
                            ...
                            >
                              ...
                              <div style={{ color: 'var(--site-text-muted)', fontSize: '12px' }}>
                                Progress: {Number(entry.progressPercent || 0)}% ·{' '}
                                {entry.createdAt
