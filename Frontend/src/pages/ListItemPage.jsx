      ...
      <div className="list-progress" aria-label="Listing form progress">
        {STEPS.map((label, idx) => {
          ...
          return (
            <div
              key={label}
              className={`list-progress-step ${active ? 'active' : ''} ${done ? 'done' : ''}`}
            >
              <span>{number}</span>
              <small>{label}</small>
            </div>
      ...
      <form className="list-form" onSubmit={handleSubmit}>
        ...
        {step === 3 && (
          <section>
            ...
            <p className="hint">
              Images are currently sent as inline data URLs for the draft flow.
            </p>
        ...
        {step === 4 && (
          <section>
            ...
            <div className="provenance-preflight">
              ...
              {!provenanceResult ? (
                <p className="hint">
                  Run a preflight check to scan internal hashes and reverse-image duplicate signals.
                </p>
              ) : (
                <div
                  className={`provenance-result ${provenanceResult.isDuplicateLikely ? 'is-risk' : 'is-safe'}`}
                >
                  <div className="provenance-result-top">
                    <span className="provenance-badge">
                      {provenanceResult.isDuplicateLikely
                        ? 'Risk detected'
                  ...
                  {Array.isArray(provenanceResult.duplicates) &&
                  provenanceResult.duplicates.length > 0 ? (
                  ...
                  {provenanceResult?.reverseImage?.enabled ? (
                    <p className="hint">
                      Reverse image:{' '}
                      {provenanceResult.reverseImage.checked ? 'checked' : 'not checked'}; top score{' '}
                      {Number(provenanceResult.reverseImage.score || 0).toFixed(2)}
                    </p>
                  ) : (
                    <p className="hint">
                      Reverse image provider not configured; internal hash checks still ran.
                    </p>
                  )}
                </div>
              )}
        ...
        {syndicationSummary && (
          <div className="form-success" role="status" aria-live="polite">
                ...
                {syndicationSummary.jobs.map((job) => (
                  <div key={job.channel} className="syndication-job-row">
                    <div className="syndication-job-main">
                      ...
                      <span className={`syndication-job-status is-${job.status}`}>
                        {job.status}
                      </span>
                    </div>
            ...
            {!!syndicationSummary.jobs?.some((job) =>
              ['failed', 'manual_required'].includes(job.status),
            ) && (
