...
export default function UserDashboard() {
  ...
  async function loadDashboardData() {
    try {
      ...
      const [ordersR, itemsR, txR, mpR, escrowR, salesR, dealsR, contactsR, templatesR, streamsR] =
        results;
      ...
      setDashboardData({
        ...
        userItems:
          itemsR.status === 'fulfilled'
            ? Array.isArray(itemsR.value?.items)
              ? itemsR.value.items
              : []
            : [],
        userTransactions: txR.status === 'fulfilled' ? txItems : [],
        deals:
          dealsR.status === 'fulfilled' && Array.isArray(dealsR.value?.items)
            ? dealsR.value.items
            : [],
        contacts:
          contactsR.status === 'fulfilled' && Array.isArray(contactsR.value?.items)
            ? contactsR.value.items
            : [],
        templates:
          templatesR.status === 'fulfilled' && Array.isArray(templatesR.value?.items)
            ? templatesR.value.items
            : [],
        streams:
          streamsR.status === 'fulfilled' && Array.isArray(streamsR.value?.items)
            ? streamsR.value.items
            : [],
        ...
        salesMetrics:
          salesR.status === 'fulfilled'
            ? salesR.value
            : { totalSales: 0, thisMonth: 0, thisWeek: 0 },
        loading: false,
        error: null,
      });
    } catch (error) {
  ...
  const quickActions = [
    { to: '/books', label: 'Books', detail: 'Review the public-facing book launch page' },
    {
      to: '/studio',
      label: 'Writing Studio',
      detail: 'Notes, blog posts, archive publishing, social launch',
    },
    ...
    { to: '/deals', label: 'Deals', detail: `${dashboardData.deals.length} recent loaded` },
    {
      to: '/contacts',
      label: 'Contacts',
      detail: `${dashboardData.contacts.length} recent loaded`,
    },
    {
      to: '/templates',
      label: 'Templates',
      detail: `${dashboardData.templates.length} recent loaded`,
    },
  ...
  const draftStatusLabel =
    commandNoteStatus === 'typing'
      ? 'Saving...'
      : commandNoteStatus === 'save-failed'
        ? 'Save failed locally'
        : 'Saved locally';
  ...
  return (
    <div className="user-dashboard-container">
      <header className="dashboard-header">
        ...
        <p>
          Your operator workspace for pure life knowledge, writing, deals, recovery, listings, and
          live business flow
        </p>
      </header>
      <AdminNav />
      ...
      <section className="dashboard-atlas">
        ...
        <p className="dashboard-atlas-copy">
          Keep the private flow tied to the same paths as the public site so you can move naturally
          between reading, recovering, selling, and managing.
        </p>
                  ...
                  <div className="command-section-header">
                    <div>
                      ...
                      <p className="section-helper">
                        This draft pad autosaves in your browser so you can think in plain text and
                        keep moving.
                      </p>
                    </div>
                    <span className={`command-note-status ${commandNoteStatus}`}>
                      {draftStatusLabel}
                    </span>
                  </div>
                  <textarea
                ...
                <section className="overview-section command-actions-panel">
                  <div className="command-section-header">
                    <div>
                      ...
                      <p className="section-helper">
                        Jump directly into the surfaces that keep the whole system moving.
                      </p>
                    </div>
                  </div>
              ...
              <div className="stats-grid">
                ...
                <div className="stat-card">
                  ...
                  <span className="stat-value">
                    ${dashboardData.salesMetrics.totalSales?.toFixed(2) || '0.00'}
                  </span>
                ...
                <div className="stat-card">
                  ...
                  <span className="stat-value">
                    ${dashboardData.salesMetrics.thisMonth?.toFixed(2) || '0.00'}
                  </span>
                  ...
                  <div className="command-list-stack">
                    {dashboardData.deals.slice(0, 3).map((deal) => (
                      <NavLink key={deal._id} to="/deals" className="command-list-item">
                        ...
                        <span>
                          {deal.status || 'unknown'} · {deal.currency || 'USD'}{' '}
                          {deal.totalAmount || 0}
                        </span>
                      </NavLink>
                    ))}
                    {dashboardData.deals.length === 0 ? (
                      <p className="empty-state compact">No recent deals loaded yet.</p>
                    ) : null}
                  </div>
                </section>
              </div>
                    ...
                    {dashboardData.userTransactions.slice(0, 5).map((tx, i) => (
                      <div key={i} className="activity-item">
                        ...
                        <span className="activity-amount">
                          ${toAmount(tx.amount || 0).toFixed(2)}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="mp-stats">
                  ...
                  <div className="mp-stat">
                    ...
                    <p className="stat-big">
                      ${dashboardData.marketplaceStats.totalSales?.toFixed(2) || '0.00'}
                    </p>
                  </div>
                </div>
              </div>
                ...
                <section className="overview-section">
                  ...
                  <div className="command-list-stack">
                    ...
                    {dashboardData.contacts.length === 0 ? (
                      <p className="empty-state compact">No contacts loaded yet.</p>
                    ) : null}
                  </div>
                </section>
                ...
                <section className="overview-section">
                  ...
                  <div className="command-list-stack">
                    ...
                    {dashboardData.templates.length === 0 ? (
                      <p className="empty-state compact">No templates loaded yet.</p>
                    ) : null}
                  </div>
                </section>
              </div>
            ...
            <section className="tab-content orders-content">
              ...
              {dashboardData.userOrders.length > 0 ? (
                  ...
                  {dashboardData.userOrders.map((order, i) => (
                    <div key={i} className="order-card">
                      <div className="order-header">
                        ...
                        <span
                          className={`order-status status-${(order.status || order.paymentStatus || 'pending').toLowerCase()}`}
                        >
                      ...
                      <div className="order-details">
                        <p>
                          <strong>Item:</strong>{' '}
                          {order.itemName || order.itemSnapshot?.name || 'Unknown'}
                        </p>
                        <p>
                          <strong>Amount:</strong> $
                          {(
                            (order.totalPrice ?? Number(order.amountTotal || 0) / 100) ||
                            0
                          ).toFixed(2)}
                        </p>
                        <p>
                ...
                <div className="empty-state">
                  <p>
                    No orders yet. <NavLink to="/marketplace">Browse the marketplace</NavLink>
                  </p>
                </div>
              )}
            </section>
            ...
            <section className="tab-content items-content">
              ...
              {dashboardData.userItems.length > 0 ? (
                  ...
                  {dashboardData.userItems.map((item, i) => (
                    <div key={i} className="item-card">
                      ...
                      <div className="item-info">
                        ...
                        <NavLink to={`/marketplace/${item.slug || item._id}`} className="view-btn">
                          View
                        </NavLink>
                      </div>
                    </div>
                ...
                <div className="empty-state">
                  <p>
                    No items listed yet.{' '}
                    <NavLink to="/items/new">Create your first listing</NavLink>
                  </p>
                </div>
              )}
            </section>
                ...
                <div className="transactions-list">
                  {dashboardData.userTransactions.map((tx, i) => (
                    <div key={i} className="transaction-card">
                      ...
                      <span
                        className={`tx-amount ${tx.type?.toLowerCase() === 'sale' ? 'income' : 'expense'}`}
                      >
                        {tx.type?.toLowerCase() === 'sale' ? '+' : '-'} $
                        {toAmount(tx.amount || 0).toFixed(2)}
                      </span>
                    </div>
                  ))}
                  ...
                  {dashboardData.escrowStatus.map((escrow, i) => (
                    <div key={i} className="escrow-card">
                      <div className="escrow-header">
                        ...
                        <span className={`escrow-status status-${escrow.status?.toLowerCase()}`}>
                          {escrow.status}
                        </span>
                      </div>
                      <div className="escrow-details">
                        <p>
                          <strong>Item:</strong> {escrow.itemName || 'Unknown'}
                        </p>
                        <p>
                          <strong>Amount:</strong> $
                          {(
                            Number((escrow.totalPrice ?? escrow.amount) || 0) /
                            (Number((escrow.totalPrice ?? escrow.amount) || 0) > 1000 ? 100 : 1)
                          ).toFixed(2)}
                        </p>
                        <p>
                          <strong>Release Date:</strong>{' '}
                          {new Date(escrow.releaseDate || Date.now()).toLocaleDateString()}
                        </p>
            ...
            <section className="tab-content sales-content">
              ...
              <div className="sales-metrics">
                ...
                <div className="metric-card">
                  ...
                  <p className="metric-value">
                    ${dashboardData.salesMetrics.thisWeek?.toFixed(2) || '0.00'}
                  </p>
                </div>
                <div className="metric-card">
                  ...
                  {topOrderItems.map((item) => (
                    <div key={item.name} className="top-item">
                      ...
                      <span className="item-sales">
                        ${item.revenue.toFixed(2)} ({item.count})
                      </span>
                    </div>
                  ))}
