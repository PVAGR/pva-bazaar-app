...
export default function ShowroomItemPage() {
  ...
  if (loading) return <div className="showroom-item-page"><div className="loading">Loading...</div></div>;
  if (error || !item) return <div className="showroom-item-page"><div className="error">{error || "Item not found"}</div></div>;
  ...
  return (
    <div className="showroom-item-page">
      <Helmet>
        ...
        <meta property="og:description" content={item.description || "Showroom item on PVA Bazaar"} />
        ...
        <section className="info-section">
          ...
          <div className="item-status-bar">
            ...
            <span className="item-uniqueness-indicator">
              {catalog.isUnique ? "🔷 One-of-One Piece" : `📦 Bulk Inventory — ${catalog.bulkQuantity || 0} Available`}
            </span>
          </div>
          ...
          <div className="sticky-cta-wrapper">
            <button className="sticky-cta" onClick={() => document.querySelector('.showroom-inquiry-form')?.scrollIntoView({ behavior: 'smooth' })}>
