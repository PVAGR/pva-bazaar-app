...
export default function TemplatesPage() {
  ...
  function sendViaEmail(contact, body) {
    ...
    window.open(
      `mailto:${to}?subject=${encodeURIComponent('Supply inquiry')}&body=${encodeURIComponent(body)}`,
      '_blank',
    );
    setUseWithContactModal(null);
  }
  ...
  return (
    <div className="templates-shell admin-page authenticated">
      <header className="admin-header templates-header">
        <div className="templates-header__row">
          <div>
            ...
            <p className="muted">
              Copy-paste vetting prompts, intro emails, and pitches. One-click copy or send via
              WhatsApp/Email.
            </p>
          </div>
      ...
      <main className="templates-main">
        {error ? (
          <ErrorBanner message={error} onRetry={loadTemplates} onDismiss={() => setError('')} />
        ) : null}
        ...
        <section className="card">
          ...
          <form className="form" onSubmit={handleCreate}>
            ...
            <input
              ...
              placeholder="e.g. Universal Coffee Vetting"
            />
            <label>Type</label>
            <select
              value={draft.type}
            ...
            <textarea
              ...
              placeholder="Full text (copy-paste content)..."
            />
            <div className="row">
        ...
        <section className="card">
          ...
          {!loading && items.length === 0 ? (
            <div className="muted">No templates yet. Run the seed script or create one.</div>
          ) : null}
          <div className="templates-list">
            {items.map((t) => (
              <div key={t._id} className={`template-item ${selectedId === t._id ? 'active' : ''}`}>
                <button className="template-item__btn" onClick={() => setSelectedId(t._id)}>
                  <div className="template-title">{t.name}</div>
                  <div className="muted small">{t.type}</div>
                </button>
                <button
                  className="btn ghost template-copy"
            ...
            <div className="form">
              ...
              <div className="row">
                <button
                  ...
                  onClick={() => copyToClipboard({ _id: selected._id, body: draft.body })}
                >
                  Copy to clipboard
                </button>
                <button
                  className="btn primary"
                  onClick={() => useWithContact({ _id: selected._id, body: draft.body })}
                >
                  Use with contact
                </button>
        ...
        {useWithContactModal ? (
          <section className="card modal-overlay">
            ...
            <p className="muted small">
              Pick a contact to open WhatsApp or email with the template pre-filled.
            </p>
              ...
              {contacts.map((c) => (
                <div key={c._id} className="row rowWrap">
                  ...
                  {c.whatsapp || c.phone ? (
                    <button
                      className="btn ghost"
                      onClick={() => sendViaWhatsApp(c, useWithContactModal.template?.body || '')}
                    >
                      WhatsApp
                    </button>
                  ) : null}
                  {c.email ? (
                    <button
                      className="btn ghost"
                      onClick={() => sendViaEmail(c, useWithContactModal.template?.body || '')}
                    >
                      Email
                    </button>
                  ) : null}
            ...
            <button className="btn ghost" onClick={() => setUseWithContactModal(null)}>
              Cancel
            </button>
          </section>
        ) : null}
