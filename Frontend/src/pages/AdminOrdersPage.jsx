...
function formatCents(c, cur) {
  ...
  return new Intl.NumberFormat(undefined, { style: 'currency', currency: cur || 'USD' }).format(
    c / 100,
  );
}
...
export default function AdminOrdersPage() {
  ...
  function isDirty() {
    ...
    return (
      ...
      (typeof edit.trackingNumber === 'string' &&
        edit.trackingNumber !== orderDetail.trackingNumber) ||
  ...
  async function handleSave() {
    ...
    try {
      const patch = {};
      if (edit.fulfillmentStatus && edit.fulfillmentStatus !== orderDetail.fulfillmentStatus)
        patch.fulfillmentStatus = edit.fulfillmentStatus;
      if (typeof edit.adminNotes === 'string' && edit.adminNotes !== orderDetail.adminNotes)
        patch.adminNotes = edit.adminNotes;
      if (
        typeof edit.trackingNumber === 'string' &&
        edit.trackingNumber !== orderDetail.trackingNumber
      )
        patch.trackingNumber = edit.trackingNumber;
      if (typeof edit.carrier === 'string' && edit.carrier !== orderDetail.carrier)
        patch.carrier = edit.carrier;
  ...
  async function handleRefund(full = false) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // update list row
        setOrders((prev) =>
          prev.map((o) => (o._id === orderDetail._id ? { ...o, refundStatus: res.status } : o)),
        );
      } else {
    ...
    <div className="admin-orders-page">
          ...
          <tbody>
            {orders.map((order) => (
              <tr
                ...
                className={order._id === selectedOrderId ? 'selected' : ''}
              >
      ...
      {selectedOrderId && (
        <div
          ...
          onClick={(e) => {
            if (e.target.classList.contains('order-detail-overlay')) setSelectedOrderId(null);
          }}
        >
            ...
            {detailLoading ? (
              ...
              orderDetail && (
                <>
                  ...
                  <div>
                    <b>Created:</b> {formatDate(orderDetail.createdAt)}
                  </div>
                  <div>
                    <b>Item:</b> {orderDetail.itemSnapshot?.name} ({orderDetail.itemSnapshot?.slug})
                  </div>
                  <div>
                    <b>Amount:</b> {formatCents(orderDetail.amountTotal, orderDetail.currency)}
                  </div>
                  <div>
                    <b>Payment Status:</b> {orderDetail.paymentStatus}
                  </div>
                  <div>
                    <b>Refund Status:</b> {orderDetail.refundStatus}
                  </div>
                  <div>
                    <b>Customer:</b> {orderDetail.customerEmail}{' '}
                    {orderDetail.customerName && `(${orderDetail.customerName})`}
                  </div>
                  <div>
                    <b>Stripe Session:</b> {orderDetail.stripeSessionId}
                  </div>
                  <div>
                    <b>Payment Intent:</b> {orderDetail.stripePaymentIntentId}
                  </div>
                  {orderDetail.stripeRefundId && (
                    <div>
                      <b>Refund ID:</b> {orderDetail.stripeRefundId}
                    </div>
                  )}
                  {orderDetail.refundAmountCents && (
                    <div>
                      <b>Refunded:</b>{' '}
                      {formatCents(orderDetail.refundAmountCents, orderDetail.currency)}
                    </div>
                  )}
                  {orderDetail.refundedAt && (
                    <div>
                      <b>Refunded At:</b> {formatDate(orderDetail.refundedAt)}
                    </div>
                  )}
                  <div>
                    <b>Shipping:</b>{' '}
                    {orderDetail.shipping ? JSON.stringify(orderDetail.shipping) : '-'}
                  </div>
                  <div className="detail-actions">
                    ...
                    <div className="fulfillment-editor">
                      <label>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setEdit((edit) => ({ ...edit, fulfillmentStatus: e.target.value }))
                          }
                      ...
                      <label>
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setEdit((edit) => ({ ...edit, adminNotes: e.target.value }))
                          }
                          rows={2}
                          disabled={saveLoading}
                      ...
                      <label>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEdit((edit) => ({ ...edit, trackingNumber: e.target.value }))
                          }
                          disabled={saveLoading}
                        />
                      </label>
                      <label>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setEdit((edit) => ({ ...edit, carrier: e.target.value }))
                          }
                          disabled={saveLoading}
                        />
                      </label>
                    ...
                    {orderDetail.paymentStatus === 'paid' &&
                      orderDetail.refundStatus !== 'refunded' && (
                        <>
                          <button
