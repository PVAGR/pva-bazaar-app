  ...
  return (
    <article className="gov-card">
      <div className="gov-card-meta">
        ...
        <span className="gov-tag gov-tag-cat">
          {proposal.committeeCategory || proposal.category || 'Governance'}
        </span>
        {String(proposal.urgency || '').toLowerCase() === 'high' ? (
          <span className="gov-pill-urgent">Urgent</span>
        ) : null}
      </div>

      <h3 className="gov-card-title">{proposal.title}</h3>
      <p className="gov-card-excerpt">
        <strong>Problem:</strong> {proposal.problem || 'Not provided.'}
      </p>
      <p className="gov-card-excerpt">
        <strong>Proposal:</strong> {proposal.proposal || proposal.solution || 'Not provided.'}
      </p>
      <p className="gov-card-excerpt">
        <strong>Expected outcome:</strong>{' '}
        {proposal.expectedOutcome || proposal.outcome || 'Not provided.'}
      </p>
      {proposal.costResources ? (
        <p className="gov-card-excerpt">
          <strong>Cost/resources:</strong> {proposal.costResources}
        </p>
      ) : null}
      ...
      {proposal.adminDecision ? (
        <div className="gov-card" style={{ margin: '0.75rem 0', borderStyle: 'dashed' }}>
          <div className="gov-card-meta">
            <span className={statusTagClass(proposal.adminDecision)}>
              {statusLabel(proposal.adminDecision)}
            </span>
            <span className="gov-tag">Public moderator decision</span>
          </div>
          {proposal.adminReason ? (
            <p className="gov-card-excerpt">
              <strong>Reason:</strong> {proposal.adminReason}
            </p>
          ) : null}
          {proposal.nextStep ? (
            <p className="gov-card-excerpt">
              <strong>Next step:</strong> {proposal.nextStep}
            </p>
          ) : null}
          {proposal.targetTimeline ? (
            <p className="gov-card-excerpt">
              <strong>Target timeline:</strong> {proposal.targetTimeline}
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      {proposal.executionProject ? (
        <div className="gov-card" style={{ margin: '0.75rem 0', borderStyle: 'dashed' }}>
          ...
          <p className="gov-card-excerpt">
            <strong>Owner:</strong> {proposal.executionProject.owner || 'Unassigned'}
          </p>
          {proposal.executionProject.latestUpdate ? (
            <p className="gov-card-excerpt">
              <strong>Latest update:</strong> {proposal.executionProject.latestUpdate}
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      <p className="gov-card-excerpt">
        <strong>Discussion:</strong> {comments.length} comment(s)
      </p>
      {latestComment ? (
        <p className="gov-card-excerpt">
          <strong>Latest:</strong> {latestCommentAuthor}: {latestCommentBody}
        </p>
      ) : null}

      <div className="gov-card-footer">
        <div className="gov-card-author">
          <span className="gov-avatar" style={{ background: proposal.authorColor || '#4a90a4' }}>
            {proposal.authorInitial || 'C'}
          </span>
