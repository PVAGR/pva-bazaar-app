...
export default function PVAFood() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      setError('');
      const response = await fetchArchiveEntries({ q: 'food recipe kitchen', limit: 8, sort: 'new' });
      if (cancelled) return;
      if (response.ok) {
  ...
  return (
    <section>
      ...
      {!loading && !error && items.length === 0 && <p>No food-related entries were found in archive data.</p>}
