...
// middleware MUST run before the general JSON parser below. Mirrors the
// full-server entry (backend/api/index.js).
const stripeWebhookPaths = new Set(['/webhooks/stripe', '/api/webhooks/stripe']);
function stripeWebhookRawBody(req, res, next) {
  if (stripeWebhookPaths.has(req.originalUrl)) {
...
// Canonical commerce path (Phase 3): listings resolve prices server-side,
// checkout creates server orders, inquiries persist server-side. These routers
// carry their own auth/ownership checks; mounting follows the existing pattern.
app.use('/api/checkout', require('../routes/checkout'));
app.use('/api/orders', require('../routes/orders'));
app.use('/api/item-inquiries', require('../routes/itemInquiries'));
app.use('/api/sales', require('../routes/sales'));
app.use('/api/seller', require('../routes/seller'));
app.use('/api/products', require('../routes/products'));
// Stripe webhook handler (Phase 3): production relied only on the
// finalize-session fallback because the serverless entry never mounted the
// webhook route. Reuse the existing handler; raw body is captured by
// stripeWebhookRawBody above (before the JSON parser) so Stripe signature
// verification works. finalize-session remains as the idempotent fallback.
app.use('/webhooks', require('../routes/webhooksStripe'));
app.use('/api/webhooks', require('../routes/webhooksStripe'));
