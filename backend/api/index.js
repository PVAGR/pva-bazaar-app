...
const meowRoutes = require('../routes/meow');
...
// Consolidated readiness verdict for fast monitoring/checks.
app.get('/api/decentralized/ready', async (req, res) => {
  const rpcDiagnostics = await getRpcDiagnostics({ timeoutMs: 3000 });
  const checks = {
    rpcConfigured: rpcDiagnostics.configured,
    rpcReachable: rpcDiagnostics.reachable,
