...
async function main() {
  const ollamaBaseUrl = trim(
    process.env.OLLAMA_BASE_URL || process.env.OPENCLAW_OLLAMA_BASE_URL || '',
  );
  const ollamaModel = String(
    process.env.OLLAMA_MODEL || process.env.OPENCLAW_OLLAMA_MODEL || 'llama3.1',
  ).trim();
  const backendBaseUrl = trim(
    process.env.OPENCLAW_BACKEND_URL || process.env.BACKEND_BASE_URL || 'https://api.pvabazaar.org',
  );
  ...
  if (!ollamaBaseUrl) {
    ...
    try {
      ...
      console.log(
        `ollamaVersionCheck=ok status=${version.status} version=${version.data?.version || 'unknown'}`,
      );
    } catch (err) {
      console.log(
        `ollamaVersionCheck=failed status=${err?.response?.status || 'n/a'} detail=${(err?.response?.data?.error || err.message || 'unknown').toString().slice(0, 240)}`,
      );
    }
    ...
    try {
      ...
      console.log(
        `ollamaChatCheck=failed status=${err?.response?.status || 'n/a'} detail=${(typeof detail === 'string' ? detail : JSON.stringify(detail)).slice(0, 300)}`,
      );
    }
  }
  ...
  try {
    ...
    console.log(
      `openclawStatus=ok mode=${payload.mode || 'unknown'} webhookConfigured=${Boolean(payload.webhookConfigured)} message=${String(payload.message || '').slice(0, 180)}`,
    );
  } catch (err) {
    console.log(
      `openclawStatus=failed status=${err?.response?.status || 'n/a'} detail=${(err?.response?.data?.message || err.message || 'unknown').toString().slice(0, 240)}`,
    );
  }
}
