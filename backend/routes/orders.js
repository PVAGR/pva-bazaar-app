...
router.get('/escrow', authenticateToken, async (req, res) => {
  try {
    ...
    const orders = await Order.find({
      $or: [{ buyerId: req.user.id }, { 'attribution.creatorId': req.user.id }],
    })
      .sort({ createdAt: -1, _id: -1 })
      .limit(limit)
      .lean();
...
router.post('/:id/refund', requireAdmin, async (req, res) => {
  try {
    ...
    if (!order.stripePaymentIntentId)
      return res.status(400).json({ ok: false, error: 'No payment intent' });
    if (order.refundStatus === 'pending' || order.refundStatus === 'refunded')
      return res.status(400).json({ ok: false, error: 'Order already refunded or refund pending' });
...
router.put('/:id', requireAdmin, async (req, res) => {
  try {
    ...
    if (
      fulfillmentStatus &&
      ['unfulfilled', 'processing', 'shipped', 'delivered'].includes(fulfillmentStatus)
    ) {
...
router.get('/ops/omnichannel', requireAdmin, async (req, res) => {
  try {
    ...
    const [sales, pendingCryptoOrders] = await Promise.all([
      OmnichannelSale.find(saleFilter).sort({ createdAt: -1, _id: -1 }).limit(limit).lean(),
      Order.find({
  ...
  try {
    ...
    const [
      ...
      OmnichannelSale.aggregate([
        {
          $group: {
            ...
            totalSales: { $sum: 1 },
            creatorRoyaltyCents: {
              $sum: { $ifNull: ['$royaltySettlement.creatorRoyaltyCents', 0] },
            },
      ...
      OmnichannelSale.find({ 'royaltySettlement.amountCents': { $gt: 0 } })
        ...
        .lean(),
      ProvenanceReviewLog.find({}).sort({ createdAt: -1, _id: -1 }).limit(limit).lean(),
    ]);
