...
router.post('/setup', async (req, res) => {
  try {
    ...
    if (!page) {
      ...
      await page.save();
      return res.json({ ok: true, slug, editSecret: newSecret, message: 'Page created. Keep the secret safe!' });
    }
    ...
    return res.json({ ok: true, slug, exists: true, message: 'Page already exists. Use rotate=true to issue a new secret.' });
  } catch (err) {
...
router.get('/:slug', async (req, res) => {
  try {
    ...
    res.json({ ok: true, slug: page.slug, title: page.title, content: page.content, updatedAt: page.updatedAt });
  } catch (err) {
