...
router.get(
  ...
  requireAdmin,
  asyncHandler(async (req, res) => {
    const agent = await AutonomousAgent.findById(req.params.agentId).select(
      '-emailPassword -bankAccount.accountNumber -bankAccount.routingNumber',
    );
    ...
    const schedules = await AgentBillingSchedule.find({ agentId: agent._id }).lean();
