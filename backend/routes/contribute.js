...
router.post('/submit', async (req, res) => {
  try {
    ...
    if (!title || title.length < 3) return res.status(400).json({ ok: false, message: 'Title too short' });
    if (!content || content.length < 5) return res.status(400).json({ ok: false, message: 'Content too short' });
    ...
    const blog = new Blog({ slug, title, content, authorName, status: 'pending', editHashHashed: null });
    await blog.save();
