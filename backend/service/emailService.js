...
async function sendConsignmentEmail({ to, subject, itemData, status }) {
  ...
  const statusMessages = {
    pending_review:
      'Your item is pending review by our team. We will notify you once it has been reviewed.',
    approved: 'Congratulations! Your item has been approved and is now live on the marketplace.',
    rejected:
      'Unfortunately, your item did not meet our requirements. Please review our guidelines and try again.',
 ...
 */
async function sendFulfillmentConfirmationEmail({
  to,
  orderId,
  downloadToken,
  itemName,
 ...
 */
async function sendDealInitiationEmail({
  to,
  sellerName,
  sellerEmail,
  dealId,
 ...
 */
async function sendDealAcceptanceEmail({
  to,
  sellerName,
  buyerName,
  buyerEmail,
 ...
 */
async function sendMockConfirmationEmail({
  to,
  recipientName,
  dealId,
  amount,
