...
3. In the Render dashboard, set **Environment** variables marked `sync: false` (at minimum **`MONGODB_URI`** and **`JWT_SECRET`**). If you still have a Vercel CLI export at `.tmp-vercel-pva-backend/.env.production.local`, run `powershell -File scripts/sync-render-dashboard-env.ps1` from the repo root: it writes **`render-dashboard.env`** (gitignored) that you can paste into Render’s Environment editor. Otherwise copy from [`backend/.env.example`](./backend/.env.example).
